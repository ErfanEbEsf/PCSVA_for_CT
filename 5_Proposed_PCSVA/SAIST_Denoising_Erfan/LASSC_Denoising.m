  ...
  %   end
    ...
    for  i =1 : L
        ...
        [Ys(:, blk_arr(:,i)), W(:, blk_arr(:,i)), R(i)] =...
          Low_rank_SSC((B), par.c1, nsig, mB, par.k0);
    end
    %R_save(iter,:)=R;
%     im_out   =  zeros(h,w);
    im_wei   =  zeros(h,w);
...
% Sigma0            =   diag(Sigma0);
% S = zeros(size(Sigma0));
% C1 = Sigma0 - eps; C2 = (Sigma0 + eps).^2 - 4*c1;
...
% YY = data_noisy * W;
% 
% [U,SigmaY,V] =   svd(full(YY),'econ');    
%     PatNum       = size(Y,2);
%     TempC  = c1*sqrt(PatNum)*2*nsig^2;
%     temp=(SigmaY-eps).^2-4*(TempC-eps*SigmaY);
%     ind=find (temp>0);
%     svp=length(ind);
%     SigmaX=max(SigmaY(ind)-eps+sqrt(temp(ind)),0)/2;
%     YY =  U(:,1:svp)*diag(SigmaX)*V(:,1:svp)';
% 
% X = YY * W';
% X = X';
% wei = 100/(sum(E(1:k))*nsig^2); %This is highly in line with BM3D's rule of weight proportionality with reciprocal of total sample variance
